...
PURPOSE:
  ...
  With wild bootstrapping option.
"""
...
def run_ppxf(
    ...
    try:
        ...
        # best-fit from last run
        bestfit0 = pp.bestfit.copy()
        resid = log_bin_data - bestfit0
        for o in range(0, nsims):
            # Add noise to bestfit according to Cappellari bootstrapping prescription:
            noisy_bestfit = bootstrap_residuals(bestfit0, resid)
            mc = ppxf(
                templates,
                noisy_bestfit,
                log_bin_error,
